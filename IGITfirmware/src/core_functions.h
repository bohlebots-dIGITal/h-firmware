...
void action() {
  ...
  int speed = 0;
  int turn = 0;
  bool shouldKick = (gotBall && abs(goalDirection) < 20 &&
  ...
  if (kickOff) {
    kickOff = false;
    igitBot.drive(0, (100 * SPEED_PERCENT) / 100, -15);
    ...
    if (goalVisible) {
      cornerTimer = 0;
      turn = goalDirection * 0.9;  // wenn Pixy tor sieht in torrichtung drehen
    } else if (ownGoalVisible) {
      turn = 40;
    } else {
      if (isInCorner()) {
        getOutOfCorner();
