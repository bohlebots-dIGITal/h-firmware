...
void action()
{
  ...
  if (goal_seen || !gotBall)
    corner_timer = 0;

  if (goal_seen)
    turn = goalDir; // wenn Pixy tor sieht in torrichtung drehen

  if (corner_timer > 1000)
  { // bot steht  in der ecke
    if (goalSide == Right)
      turn = 7;
    else
      turn = -7;
  ...
  { // tor wird nicht gesehen aber nicht in der ecke
    spd = 40;
    if (whatWeWorkWith == 0)
      turn = 0;
    else
      turn = (-igitBot.kompass() / 5) /*-(side(dir*2)*10)*/;
  }
